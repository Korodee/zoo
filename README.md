Domaine du Chevreuil Blanc is a visionary wildlife park project founded by Denis Desjardins in L'Ange-Gardien, Outaouais, Quebec. It's not just a zoo—it's a community-driven initiative that combines wildlife conservation with an innovative membership-based prize system that rewards participants with substantial rewards. The frontend is built with Next.js 14, Typescript and Tailwind CSS. The backend is an Express API using MongoDB, JWT authentication, and Stripe payments. Swagger is provided for API documentation. The app is designed to deploy on Vercel (frontend) and Vercel (backend).
